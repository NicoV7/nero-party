...
function compareLeaderboardSongs(a: LeaderboardSongData, b: LeaderboardSongData): number {
...
export async function buildFinalResults(partyId: string): Promise<LeaderboardSongData[]> {
  ...
  return [...leaderboard].sort(compareLeaderboardSongs);
}
